...
from lagou_spider.utils import color
from scrapy.utils.misc import arg_to_iter
from twisted.internet.defer import Deferred, DeferredList
from lagou_spider.utils.select_result import list_first_item
...
class lagouBookFile(FilePipeline):
    ...
    def __init__(self,store_uri,download_func=None):
        super(lagouBookFile, self).__init__(store_uri=store_uri,download_func=download_func)
        if not store_uri:
            raise NotConfigured
    ...
    def another_process_item(self, result, item, info):
        ...
        assert isinstance(result, (Item, Request)), \
                    "lagouBookFile pipeline' item_completed must return Item or Request, got %s" % \
                    (type(result))
